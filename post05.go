...
func exists(cname string) string {
	...
	courseID := "no"
	statement := fmt.Sprintf(`SELECT "cid" FROM "MSDSCourseCatalog" where cname = '%s'`, cname)
...
func AddCourse(d MSDSCourse) string {
	...
	insertStatement = `insert into "MSDSCourse" ("courseid", "cname", "cprereq")
	values ($1, $2, $3, $4)`
...
func ListCourses() ([]MSDSCourse, error) {
	...
	rows, err := db.Query(`SELECT "cid","cname","cprereq"
		FROM "MSDSCourseCatalog","MSDSCourse"
		WHERE MSDSCourseCatalog.id = MSDSCourse.userid`)
	if err != nil {
		return Data, err
	}
...
func UpdateCourse(d MSDSCourse) error {
	...
	d.CID = courseID
	updateStatement := `update "MSDSCourse" set "cname"=$1, "cprereq"=$2 where "courseid"=$4`
